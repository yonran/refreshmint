...
| Accept-all ML suggestions in one batch            | EX·M         | "Accept N suggestions" applies every visible uncategorized row's ML suggestion in a single confirmed batch; see `src/categorize-utils.ts`, `src/tabs/TransactionsTable.tsx`.                                                                                                                                                           |
| User-defined category rules (payee/regex → acct)  | EX·M         | `category-rule` resolutions match by normalized payee, regex, and amount bounds; matching entries post directly to the rule account and existing Unknown rows get Auto proposals; created one-click via "Always: <account>" or the bulk "Create rule" checkbox; see `src-tauri/src/automation.rs`, `src-tauri/src/payee_normalize.rs`. |
| Automation policy loop after batch posts          | EX·M         | Auto-policy proposals (rule- and resolution-derived) drain automatically after auto-ETL, Post All, and CLI `account post-all`; see `src-tauri/src/automation.rs`, `src/App.tsx`, `src-tauri/src/cli.rs`.                                                                                                                               |
