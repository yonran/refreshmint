...
| `await page.waitForEvent('popup' \| 'request' \| 'response', timeoutMs?)` | Playwright-style event waiter. `popup` returns a `Page`, `request` returns the next captured `Request`, and `response` returns the next captured `Response`.     |
