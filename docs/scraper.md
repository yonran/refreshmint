...
| `await page.waitForResponse(urlPattern, optionsOrTimeout?)`                                                       | Wait for a captured network response URL pattern and return a Playwright-style `Response` object.                                                                 |
| `await page.waitForRequest(urlPattern, optionsOrTimeout?)`                                                        | Wait for a captured network request URL pattern and return a Playwright-style `Request` object.                                                                   |
...
| `await page.waitForEvent('popup' \| 'request' \| 'response' \| 'requestfinished' \| 'requestfailed', timeoutMs?)` | Playwright-style event waiter. `popup` returns a `Page`, `request` / `requestfinished` / `requestfailed` return a `Request`, and `response` returns a `Response`. |
